...
// ------------------------ 插值函数（五次多项式） ------------------------
double computeQuinticBlend(double t, double totalTime) {
    if (t <= 0.0) {
        return 0.0;
    }
    if (t >= totalTime) {
    ...
    const double s = t / totalTime;
    const double s2 = s * s;
    const double s3 = s2 * s;
    const double s4 = s3 * s;
    const double s5 = s4 * s;
    // 五次多项式让起点/终点速度和加速度为 0，避免实时控制启停时出现阶跃。
    return 10.0 * s3 - 15.0 * s4 + 6.0 * s5;
}

double computeInterpolatedTheta(double t, double totalTime, double theta0, double theta1) {
    return theta0 + (theta1 - theta0) * computeQuinticBlend(t, totalTime);
}
...
void processRealtimeData() {
    ...
    // ===== 插值参数 =====
    const double circleDuration = 8.0;      // 画完整圆的总时长（s）
    const double dt = sampleTime / 1000.0;  // 每周期步进（s）
    double t = 0.0;                         // 当前时间（s）
    ...
    std::cout << "五次多项式圆轨迹: 半径 " << r
              << " mm, 总时长 " << circleDuration << " s。" << std::endl;

    // ===== 主控制循环：发送轨迹点 =====
    // servoToCPOS 为阻塞式发送接口；网络通信良好时，单次阻塞时间通常接近 sampleTime。
    while (t <= circleDuration) {
        double theta = computeInterpolatedTheta(t, circleDuration, theta_0, theta_1);
    ...
    // ===== 最后一点作为终点 =====
    target[0] = centerX + r * std::cos(theta_1);
    target[1] = centerY + r * std::sin(theta_1);
    target[2] = centerZ;
